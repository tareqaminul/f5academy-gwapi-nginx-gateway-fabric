...
**Option 1:** If you're running the community controller today and need a supported replacement without rewriting all your configs, NGINX Ingress Controller (NIC) is your answer. It's built and maintained by F5's dedicated NGINX team, you get continuous security updates, feature enhancements, and access to enterprise support if you need it. Most of your existing Ingress resources will work as drop-in replacements. For advanced features that currently rely on annotations, NIC offers Custom Resources like VirtualServer and VirtualServerRoute. 

<mark>A *migration guide* for this option is available [here](https://docs.nginx.com/nginx-ingress-controller/install/migrate-ingress-nginx/) and also, for *production grade migration*, please explore the Interactive YAML analysis, 130+ annotation mappings, and CRD migration examples [here](https://alessfg.github.io/nginx-ingress-migration-guide/#migration) and for *ingress2gateway* tool by [kubernetes-sigs](https://github.com/kubernetes-sigs/ingress2gateway) with [example usage](https://alessfg.github.io/nginx-ingress-migration-guide/#i2g). </mark>
